...
unigram <- data.table(readRDS("data/1_gram_trainv2.rds"))
bigram <- data.table(readRDS("data/2_gram_trainv2.rds"))
trigram <- data.table(readRDS("data/3_gram_trainv2.rds"))




backoff_trigram <- function( u, v, dis_tri = 0.5, dis_bi = 0.5) {
  uv <- paste(u, v, sep = " ")
